...
Figure \ref{fig:scale-npes} compares the communication model to actual runtime for three different random matrices, varying the PE gridsize. For the smallest matrix, with 8192 rows and
...
\textcolor{red}{
    BIG IDEA 2: how do we make decisions about layout and resources for optimal performance?
}

Mapping a given matrix to the WSE SpMV algorithm requires (1) selecting the matrix layout, and (2) selecting the PE grid size.
To analyze this second aspect, we evaluate a set of matrices from our RandomSuite at different PE gridsizes. Figure \ref{fig:randsuite-wse-heatmap} visualizes the exploration space of RandomSuite, from square matrices of dimension 8192 to 2097152 with the number of nnzs per row increasing along the y axis. As matrices get bigger and denser, the optimal grid size increases.

While this evaluation can be abstracted to more general matrices, we are interested in how well our communication model captures these trends.
...
We first evaluate the GPU across our RandomSuite space varying the number of nonzeros per row and the number of rows. Figure \ref{fig:gpu-vs-wse-varydensity} shows the throughput achieved for increasing matrix size on the H200 and WSE. \textcolor{green}{TODO: WSE RandomSuite results}
