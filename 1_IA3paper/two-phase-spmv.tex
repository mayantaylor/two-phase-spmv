...
Within each PE, matrix data is stored in CSR format and processed accordingly. Our original compute kernel is described in Algorithm \ref{alg:spmv}.
...
\begin{algorithm}
    ...
    \begin{algorithmic}[1]
        ...
        \State $sum \gets sum + \texttt{vals}[nnz] \cdot \texttt{x}[ \texttt{vals}[nnz]]$
        \EndFor
        \State $partial[r] \gets sum$
        \EndFor
    \end{algorithmic}
\end{algorithm}

\begin{algorithm}
    \caption{SpMV Kernel in CSR Format with Row-Index Materialization}
    \label{alg:spmv-rim}
    \begin{algorithmic}[1]
        \ForAll{$n \in \texttt{nnzs}$}
        \State $sum \gets \texttt{vals}[nnz] \cdot \texttt{x} \texttt{vals}[nnz]$
        \State $partial[\texttt{get_row}[n]] \gets partial[\texttt{get_row}[n]] + sum$
        \EndFor
    \end{algorithmic}
\end{algorithm}
