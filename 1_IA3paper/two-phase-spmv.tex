...
In the original presentation of these collectives, Luczynski et al. also develop a performance model for them. This performance model is based on the parameters $P$ and $B$ and an additional variable $T_{ramp}$, the ramp latency. Specifically, $T_{ramp}$ captures the latency between message arrive on a PE at the router to that message being received by the compute engine.

We build on Luczynski et al. by (1) implementing the collectives in Tungsten and Paint, and (2) updating the performance model.
...
Before building the full collectives, we evaluate a pair-wise pingpong benchmark to capture the communication latencies between PEs.
\begin{enumerate}
